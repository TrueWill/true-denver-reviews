...
const areaMap = new Map<string, string>(
  rawRows.map((r) => [r[0].toLowerCase(), r[0]]),
);
...
function matchByName(candidate: string): string | null {
  ...
  if (synonyms[lower] && areaMap.has(synonyms[lower].toLowerCase()))
...
console.log(
  `Found ${withAddress.length} places with addresses. Areas with bounds: ${areasWithBounds.length}`,
);
...
for (const place of withAddress) {
  ...
  if (found) {
    ...
    console.log(
      `  "${place.name}" → not found at (${lat},${lng}) | candidates: ${candidates.join(', ') || 'none'}`,
    );
    results.push({ name: place.name, area: '', status: 'not_found' });
  }
}
