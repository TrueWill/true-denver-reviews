...
function buildQuery(place: Place): string {
  ...
  const needsCategory = words.length < 2 || name.length < SHORT_NAME_THRESHOLD;
  const suffix =
    needsCategory && place.category ? ` ${place.category.toLowerCase()}` : '';
...
async function processPlace(
  ...
  if (passing.length >= 2 && passing[0].address !== passing[1].address) {
    return {
      ...
      google_name: passing[0].googleName,
      query_used: query,
    };
  }
  if (passing.length === 0) {
    ...
    return {
      ...
      status: 'skipped',
      reason: first.reason,
      google_name: first.googleName,
      distance_km: roundOpt(first.distanceKm),
      query_used: query,
    };
  }
  ...
  return {
    ...
    distance_km: roundOpt(best.distanceKm),
    query_used: query,
  };
}
